  ...
  private async processTask(task: Task) {
    try {
      await prisma.$transaction(async (tx) => {
        ...
        if (data === null) {
          this.logger.warn(data, 'Empty transaction data');
          return;
        }
        const price = await this.priceService.getPrice(data.tokenAddress, data.decimals);
        if (price.eq(0)) {
          this.logger.warn(`Price for token ${data.tokenAddress} is zero, skipping order ${data.orderId}`);
        ...
        await this.upsertOrder(data.orderId, data.amount, data.tokenAddress, data.decimals, usdValue, task, tx);

...
import { createOrderWithNonceInstructionDiscriminator } from '../common/generated/src/instructions'
import { fulfillOrderInstructionDiscriminator } from '../common/generated/dst/instructions'
...
export class DlnIndexer implements IWorker {
    ...
    private isTargetTransaction(tx: any): boolean {
        ...
        return instructions.some((ix: any) => {
            ...
            if(result) {
                this.logger.info({ discriminator, name: DISCRIMINATOR_NAMES[discriminator] }, 'isTargetTransaction.discriminator found')
            }
            return result
        });
    }
            ...
            await prisma.$transaction(
                async (tx) => {
                    ...
                    if (existingTask) {
                        ...
                        this.logger.debug({ signature }, 'Task updated (status preserved)');
                    } else {
                        ...
                        this.logger.debug({ signature }, 'New task created (PENDING)');
                    }
    ...
    public stop() {
        ...
        // Здесь можно закрыть prisma.$disconnect()
        process.exit(0);
    }
}
